...
export async function run(
  ...
  const path = `/repos/${owner}/${repo}/issues?state=${encodeURIComponent(state)}&per_page=${encodeURIComponent(per_page)}`;
  ...
  const summary = list.map((i: { number: number; title: string; state: string; html_url?: string }) =>
    `#${i.number} ${i.title} (${i.state}) ${i.html_url ?? ""}`
  ).join("\n");
  return { content: [{ type: "text", text: summary || "No issues." }] };
}
